...
const Sidebar = ({ isOpen, setIsSidebarOpen }) => {
  ...
  // Filter chats for the modal based on debounced search
    ...
    <>
      ...
      >
        <div className={`flex flex-col h-full w-[14rem] transition-opacity duration-300 ${isOpen ? "opacity-100" : "opacity-0"}`}>
          {/* Header Area with Search & Close Icons */}
          <div className="flex items-center justify-between mb-4 mt-1">
            <button 
              onClick={() => setIsSidebarOpen(false)}
              className="p-2 shrink-0 rounded-md hover:bg-gray-800 text-gray-400 hover:text-white transition-colors"
              title="Close Sidebar"
            >
